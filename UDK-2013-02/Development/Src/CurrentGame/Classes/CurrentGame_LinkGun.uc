 ...
 */
class CurrentGame_LinkGun extends UTBeamWeapon;
...
var array<CurrentGame_LinkGun> LinkedList;	// I made a funny Hahahahah :)
				...
				if (Victim != None && !WorldInfo.Game.GameReplicationInfo.OnSameTeam(Victim, Instigator))
				{
					...
					if(Victim.IsA('CurrentGame_HostagePawn')){
						...
						ShotDir = Normal(TestImpact.HitLocation - Location);
							...
							Victim.TakeDamage(0, Instigator.Controller, TestImpact.HitLocation, PushForce, InstantHitDamageTypes[1], TestImpact.HitInfo, self);
...
{
	local UTPawn P;
	local CurrentGame_LinkGun Link;
	...
	if (Instigator != None && (bReadyToFire() || IsFiring()))
	{
		foreach WorldInfo.AllPawns(class'UTPawn', P, Instigator.Location, WeaponLinkDistance)
		{
			...
			{
				Link = CurrentGame_LinkGun(P.Weapon);
...
function GetLinkedWeapons(out array<CurrentGame_LinkGun> LinkedWeapons)
{
	local int i;
...
{
	local array<CurrentGame_LinkGun> LinkedWeapons;
...
defaultproperties
{
	...
	WeaponProjectiles(0)=class'CurrentGame_LinkPlasma' // UTProj_LinkPowerPlasma if linked (see GetProjectileClass() )
