...
simulated function ProcessBeamHit(vector StartTrace, vector AimDir, out ImpactInfo TestImpact, float DeltaTime)
{
	...
	//WorldInfo.Game.Broadcast(self,Victim);
...
function process_beam_from_me_to_you(CaptorPawn captorP, HostagePawn hostageP){
...
simulated function UpdateBeam(float DeltaTime)
{
	...
	if(Instigator.Controller.isA('BotController')){
		BotController(Instigator.Controller).hostageTarget.Controller.GetActorEyesViewPoint(out_Location, out_Rotation);
		EndTrace = out_Location;
		AimDir = (EndTrace-StartTrace)/GetTraceRange();
		WorldInfo.Game.Broadcast(self,AimDir);
	}
	else{
